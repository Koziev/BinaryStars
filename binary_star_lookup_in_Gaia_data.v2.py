...
# The largest known separation between stars in a binary system is on the order of 1 parsec (206,265 AU) or more.
# These wide binaries are rare and are typically found in low-density regions of the Milky Way, such as the halo.
# The Alpha Centauri system, with Proxima Centauri separated from the Alpha Centauri pair by about 13,000 AU,
# is one of the most famous examples of a wide binary (or triple) system.
...
# Tables:
# 1.	J/A+A/678/A19/table1	(c)Gaia NSS and SB9 radial velocities (43 rows)
# 2.	J/A+A/678/A19/table2	(c)Gaia NSS and APOGEE radial velocities (13 rows)
# 3.	J/A+A/678/A19/table3	(c)Gaia resolved and Direct Imaging (6 rows)

vizier = Vizier(columns=['*'], row_limit=-1)  # Get all available columns
tables1 = vizier.get_catalogs('J/A+A/546/A69')

for table in tables1:
    print('TABLE name={} description={}'.format(table.meta['name'], table.meta['description']))
    print('Columns: {}'.format(table.colnames))
    print('Num. of rows: {}'.format(len(table)))
    df = table.to_pandas()
    print('-'*60)

table1 = tables1['J/A+A/546/A69/table1']
# GaiaDR3		Gaia DR3 ID
# plx	mas	Trigonometric parallax
# e_plx	 mas	Uncertainty of trigonometric parallax
# Per  yr	(n) Period
# Simbad	ask the Simbad data-base about this object
# _RA	 deg	(i) Positions, from Simbad and WDS (B/wds) (right ascension part)
# _DE	 deg	(i) Positions, from Simbad and WDS (B/wds) (declination part)
# a	  arcsec	Semi-major axis of orbit
# e_a  arcsec	(n) Uncertainty of semi-major axis (stat.error)
...
for row in table1:
    ...
    dec = float(row['_DE'])
    #parallax = row['plx']
    binary_points.append((ra, dec))
    binary_systems.append(row)


binary_kdtree = KDTree(binary_points, leaf_size=2)  # leaf_size is a tuning parameter
